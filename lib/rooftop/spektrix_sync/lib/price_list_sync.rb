        ...
        begin
          @spektrix_price_lists.each do |spektrix_price_list|
            ...
            if spektrix_price_list.prices.select {|p| !p.band.nil?}.empty?
              @logger.warn("Skipping price list #{spektrix_price_list.id} because there is no band")
              next
            end
        ...
        begin
          spektrix_price_list.prices.each_with_index do |spektrix_price, i |
            ...
            if spektrix_price.band.nil?
              @logger.warn("skipping because there is no matching band")
              next
            end
            ...
            if new_price.save!
              @logger.debug("saved #{new_price.meta_attributes[:ticket_price]}: #{spektrix_price.band.name}")
            end
          end
        rescue => e
          @logger.warn(e.to_s)
